...
| F3.7 | **`lctnEmdNm`(읍면동)은 실측 결측률 0%**이다 — **정정(2026-08-23): 0% 가 아니라 6,902건 중 1건(0.014%)이 결측**이다. 시군구까지 함께 비어 있어 폴백 5단계를 만들 수 없고, 그 1건이 유일한 `failed` 다. 따라서 `failed` 는 **사다리의 버그가 아니라 원본 결측**으로 발생할 수 있다 — 발생 시 먼저 원본 필드를 확인한다. 따라서 폴백 5단계(`시도+시군구+읍면동` 조합)는 항상 성립하며, **`failed`는 원리적으로 발생하지 않아야 한다.** 발생 시 버그로 간주하고 원인을 조사한다 | P0 |
...
### F8. 배치 운영 — 자동 실행 (2026-08-23 신설)

배치를 **깃허브 Actions cron** 으로 돌린다. 다른 프로젝트(`cheongyak-newsletter`·
`krx-signal-alerts`)와 같은 방식이라 운영 부담이 늘지 않는다.

**왜 지오코딩(M4)보다 먼저 켜는가**: 변경 이력·tombstone·낙찰가율 표본은 **시간이 쌓여야
생기는 값**이다. 오늘 켜지 않으면 그만큼 영영 못 얻는다. 좌표는 나중에 `lat is null` 대상을
일괄로 채우면 되므로 순서에 손해가 없다.

| ID | 요구사항 | 우선 |
|---|---|---|
| F8.1 | **매일 04:00 KST** 에 증분 수집 + 회차 배치를 실행한다. 깃허브 Actions 는 UTC 만 지원하므로 `0 19 * * 0-5`(UTC 일~금 = KST 월~토) 로 적는다 | P0 |
| F8.2 | **일요일 04:00 KST** 에 전량 수집 + 코드표 갱신을 실행한다(`0 19 * * 6`). **tombstone 판정 기회는 이 회차뿐**이다 (F4.2·F4.17) | P0 |
| F8.3 | 두 워크플로가 **동시에 돌지 않게** 한다(`concurrency` 그룹 + `cancel-in-progress: false`). 전량과 증분이 겹치면 tombstone 판정 기준 시각이 어긋나고, 같은 행을 두 트랜잭션이 다툰다 | P0 |
| F8.4 | 시크릿은 **깃허브 Secrets** 로만 주입한다(`ONBID_SERVICE_KEY`·`SUPABASE_DATABASE_URL`). **저장소가 공개이므로 실행 로그도 공개**다 — 워크플로는 시크릿을 출력하지 않아야 하고, 로그·아티팩트에 값이 남지 않음을 첫 실행에서 확인한다 (N4.1·N4.5) | P0 |
| F8.5 | 워크플로 권한을 **최소화**한다(`permissions: contents: read`). 배치는 저장소에 쓰지 않는다 | P0 |
| F8.6 | **실패 알림은 깃허브 기본 메일**을 쓴다. 상세 상황은 `onbid_batch_run` 에 남으므로 별도 알림 채널을 만들지 않는다. 필요해지면 그때 확장한다 | P1 |
| F8.7 | **실패한 회차를 즉시 재시도하지 않는다.** 재개 지점(F4.15)과 시도 시각(F1.16)이 이미 이월을 처리하므로, 다음 정기 실행이 이어받는다. 즉시 재시도는 같은 원인으로 또 실패하며 쿼터만 태운다 | P0 |
| F8.8 | `workflow_dispatch` 로 **수동 실행**을 지원한다. 모드·시군구·회차 예산을 입력으로 받아 재적재·부분 수집에 쓴다 | P1 |

> **일정 미준수는 정상 범위다.** 깃허브 cron 은 혼잡 시 지연되거나 건너뛸 수 있다. 배치가
> 멱등(AC2)이고 이월이 자동이므로 하루 걸러도 다음 실행이 따라잡는다 — 분 단위 정확도를
> 요구하지 않는다.
...
| N4.1 | 키 관리 | 모든 키는 `.env`(로컬)·깃허브 Secrets(CI)로만 주입하고 `.gitignore`에 등록. 코드·문서·로그·커밋에 평문 기입 금지. **설정 객체의 `repr` 에도 값이 나오지 않아야 한다** — 디버깅 중 객체를 찍는 일이 흔하고 공개 저장소의 Actions 로그는 누구나 볼 수 있다 |
...
**⚠️ 사건 식별자는 `(물건관리번호, 개찰일시, 회차)` 다 — 적재된 행 수가 아니다 (2026-08-23 실측).**
한 물건에 공매조건번호가 최대 10개 붙고(F4.1), 입찰정보 API 는 `(물건, 조건)` 으로 물어도
**그 물건의 이력을 통째로** 돌려준다. 그래서 같은 낙찰 사건이 조건 수만큼 적재된다 —
실측에서 낙찰 행 62건이 실제로는 **사건 13건 · 물건 6건**이었고, 그대로 세면 평균 4.8배
부풀며 조건번호가 많은 물건 하나가 분포를 좌우한다. 물건 수도 **물건관리번호 기준**으로 센다.
...
- [ ] AC14. **자동 배치가 7일 연속 예정대로 완주**하고 `onbid_batch_run` 에 빠진 날짜가 없다 (F8.1·F8.2)
...
| **R1** | **노출 키 폐기 미완** — 2026-08-23 재발급으로 새 키를 받아 사용처 4곳(onbid-mcp `.env`·Secrets, cheongyak-newsletter `.env`·Secrets)을 모두 교체했다. **그러나 재발급은 기존 키를 폐기하지 않는다** — 노출됐던 구 키가 온비드에 대해 여전히 `00 NORMAL_CODE` 를 반환함을 실측 확인했다(부록: API_FINDINGS). N4.2 의 목적(노출 키 무력화)은 **아직 달성되지 않았다.** 포털에서 구 키 폐기 또는 해당 활용신청 해지가 필요하다. **감수 중인 위험**: 유출 값을 아는 제3자가 온비드 쿼터를 소진하거나 우리 계정으로 호출할 수 있다 (공개 데이터 읽기 전용이라 데이터 유출·쓰기 위험은 없음) | 진행 중 |
...
| 90 | **AC5 검증 대상을 Claude Code → Claude Desktop 으로 변경** | 사용자 결정(2026-08-23). 둘 다 stdio 클라이언트라 서버 구현은 그대로지만 **등록 경로가 다르다** — `.mcp.json`(프로젝트 단위)이 아니라 `claude_desktop_config.json`(앱 단위)이다 | 2026-08-23 |
| 89 | **§8.3 사건 식별 규칙 명문화** — 낙찰 사건은 `(물건관리번호, 개찰일시, 회차)` 로 중복 제거 | 입찰정보 API 가 조건번호와 무관하게 물건 이력을 통째로 주므로 같은 사건이 조건 수만큼 적재된다. 실측 낙찰 행 62 → 사건 13 · 물건 6 (평균 4.8배 부풀림). 사용자 질문("낙찰률이 왜 이렇게 낮나")에서 발견 | 2026-08-23 |
| 88 | **F3.7 전제 정정 · AC3 달성 기록** | 읍면동 결측률이 0% 가 아니라 1건(0.014%) 있었다. 시군구까지 비어 폴백 5단계가 성립하지 않는다 — `failed` 는 사다리 버그가 아니라 원본 결측으로도 생긴다. 지오코딩 전량 실행 결과 `ok` 99.90% | 2026-08-23 |
| 87 | **§14 R1 갱신 · API_FINDINGS 에 인증키 모델 추가** | 재발급 후 실측: 키는 계정당 1개가 아니며 **재발급이 구 키를 폐기하지 않는다**. `30` 은 '폐기' 가 아니라 '해당 서비스 미등록' 이다. 사용처 4곳 교체는 완료, 구 키 폐기는 미완 | 2026-08-23 |
| 86 | **N4.1 보강 — 설정 객체 `repr` 마스킹** | Secrets 등록 후 CI 경로를 테스트로 고정하다 발견. `Settings` 가 dataclass 라 기본 `repr` 이 키를 그대로 출력했다. 공개 저장소 Actions 로그에서는 한 줄로 유출된다 | 2026-08-23 |
| 85 | **§14 R1 신설 — 서비스키 재발급 보류 기록** | N4.2 미충족을 숨기지 않고 남긴다. 사용자가 현재 키 유지를 선택했고 재발급은 별도 작업으로 미룬다. 요구사항 자체는 그대로 두고 **상태만 기록** — 재발급 시 해소 | 2026-08-23 |
| 84 | **F8 신설 — 배치 자동 실행** · AC14 추가 | 매일 04:00 KST 증분+회차, 일요일 전량+코드표를 깃허브 Actions cron 으로. **지오코딩(M4)보다 먼저 켠다** — 이력·tombstone·낙찰가율 표본은 시간이 쌓여야 생기는 값이라 미루면 영영 못 얻는다. 좌표는 나중에 일괄 backfill 이 가능하다 (2026-08-23 사용자 결정) | 2026-08-23 |
